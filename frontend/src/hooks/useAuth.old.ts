...
// Demo registration function for development when backend is not available
...
export const useAuthStatus = () => {
  ...
  // Check localStorage for fallback authentication state
  ...
  const isAuthenticated = !!profile || !!hasStoredAuth
  const user = profile || (() => {
    try {
